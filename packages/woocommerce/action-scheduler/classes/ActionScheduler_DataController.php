...
class ActionScheduler_DataController {
	...
	public static function init() {
		if ( self::is_migration_complete() ) {
			add_filter( 'action_scheduler_store_class', [ 'ActionScheduler_DataController', 'set_store_class' ], 100 );
			add_filter( 'action_scheduler_logger_class', [ 'ActionScheduler_DataController', 'set_logger_class' ], 100 );
			add_action( 'deactivate_plugin', [ 'ActionScheduler_DataController', 'mark_migration_incomplete' ] );
		} elseif ( self::dependencies_met() ) {
			Controller::init();
		}

		add_action( 'action_scheduler/progress_tick', [ 'ActionScheduler_DataController', 'maybe_free_memory' ] );
	}
