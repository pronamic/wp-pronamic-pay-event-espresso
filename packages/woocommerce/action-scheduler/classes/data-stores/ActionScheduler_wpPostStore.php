...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	protected function save_post_array( $post_array ) {
		add_filter( 'wp_insert_post_data', [ $this, 'filter_insert_post_data' ], 10, 1 );
		add_filter( 'pre_wp_unique_post_slug', [ $this, 'set_unique_post_slug' ], 10, 5 );
		...
		remove_filter( 'wp_insert_post_data', [ $this, 'filter_insert_post_data' ], 10 );
		remove_filter( 'pre_wp_unique_post_slug', [ $this, 'set_unique_post_slug' ], 10 );
	...
	protected function save_action_group( $post_id, $group ) {
		if ( empty( $group ) ) {
			wp_set_object_terms( $post_id, [], self::GROUP_TAXONOMY, false );
		} else {
			wp_set_object_terms( $post_id, [ $group ], self::GROUP_TAXONOMY, false );
		}
	}
	...
	protected function make_action_from_post( $post ) {
		...
		$group = wp_get_object_terms( $post->ID, self::GROUP_TAXONOMY, [ 'fields' => 'names' ] );
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, [ 'select', 'count' ], true ) ) {
		...
		if ( $query['status'] ) {
			$post_statuses = array_map( [ $this, 'get_post_status_by_action_status' ], (array) $query['status'] );
	 ...
	 */
	public function stake_claim( $max_actions = 10, DateTime $before_date = null, $hooks = [], $group = '' ) {
	 ...
	 */
	protected function claim_actions( $claim_id, $limit, DateTime $before_date = null, $hooks = [], $group = '' ) {
		// Set up initial variables.
		$date      = null === $before_date ? as_get_datetime_object() : clone $before_date;
		$limit_ids = ! empty( $group );
		$ids       = $limit_ids ? $this->get_actions_by_group( $group, $limit, $date ) : [];
	...
	public function init() {
		add_filter( 'action_scheduler_migration_dependencies_met', [ $this, 'migration_dependencies_met' ] );
