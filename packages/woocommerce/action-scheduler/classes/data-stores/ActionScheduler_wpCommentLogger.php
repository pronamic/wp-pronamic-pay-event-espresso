...
class ActionScheduler_wpCommentLogger extends ActionScheduler_Logger {
	...
	public function filter_comment_queries( $query ) {
		foreach ( [ 'ID', 'parent', 'post_author', 'post_name', 'post_parent', 'type', 'post_type', 'post_id', 'post_ID' ] as $key ) {
			if ( ! empty( $query->query_vars[ $key ] ) ) {
				return; // don't slow down queries that wouldn't include action_log comments anyway
			}
		}
		$query->query_vars['action_log_filter'] = true;
		add_filter( 'comments_clauses', [ $this, 'filter_comment_query_clauses' ], 10, 2 );
	}
	...
	public function init() {
		add_action( 'action_scheduler_before_process_queue', [ $this, 'disable_comment_counting' ], 10, 0 );
		add_action( 'action_scheduler_after_process_queue', [ $this, 'enable_comment_counting' ], 10, 0 );

		parent::init();

		add_action( 'pre_get_comments', [ $this, 'filter_comment_queries' ], 10, 1 );
		add_action( 'wp_count_comments', [ $this, 'filter_comment_count' ], 20, 2 ); // run after WC_Comments::wp_count_comments() to make sure we exclude order notes and action logs
		add_action( 'comment_feed_where', [ $this, 'filter_comment_feed' ], 10, 2 );

		// Delete comments count cache whenever there is a new comment or a comment status changes
		add_action( 'wp_insert_comment', [ $this, 'delete_comment_count_cache' ] );
		add_action( 'wp_set_comment_status', [ $this, 'delete_comment_count_cache' ] );
	}
