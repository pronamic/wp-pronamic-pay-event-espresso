...
class ActionScheduler_DBStore extends ActionScheduler_Store {
	...
	private function build_insert_sql( array $data, $unique ) {
		...
		$placeholders = array_map( [ $this, 'get_placeholder_for_column' ], $columns );
...
",
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		$db_server_info = is_callable( [ $wpdb, 'db_server_info' ] ) ? $wpdb->db_server_info() : $wpdb->db_version();
			...
			switch ( $query['partial_args_matching'] ) {
				...
				case 'like':
					foreach ( $query['args'] as $key => $value ) {
						...
						$json_partial = $wpdb->esc_like( trim( wp_json_encode( [ $key => $value ] ), '{}' ) );
	...
	public function get_claim_count() {
		...
		$sql = $wpdb->prepare( $sql, [ self::STATUS_PENDING, self::STATUS_RUNNING ] ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
