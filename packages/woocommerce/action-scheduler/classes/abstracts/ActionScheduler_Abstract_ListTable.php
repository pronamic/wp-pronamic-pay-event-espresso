...
abstract class ActionScheduler_Abstract_ListTable extends WP_List_Table {
	...
	protected function get_bulk_actions() {
		...
		foreach ( $this->bulk_actions as $action => $label ) {
			if ( ! is_callable( [ $this, 'bulk_' . $action ] ) ) {
	...
	protected function process_bulk_action() {
		...
		$method = 'bulk_' . $action;
		if ( array_key_exists( $action, $this->bulk_actions ) && is_callable( [ $this, $method ] ) && ! empty( $_GET['ID'] ) && is_array( $_GET['ID'] ) ) {
	...
	public function extra_tablenav( $which ) {
		...
		submit_button( esc_html__( 'Filter', 'action-scheduler' ), '', 'filter_action', false, [ 'id' => 'post-query-submit' ] );
		echo '</div>';
	}
	...
	protected function process_row_actions() {
		$parameters = [ 'row_action', 'row_id', 'nonce' ];
	...
	protected function display_filter_by_status() {
		...
		if ( ! isset( $this->status_counts['all'] ) ) {
			...
			$this->status_counts = [ 'all' => $all_count ] + $this->status_counts;
		}
		...
		foreach ( $this->status_counts as $status_slug => $count ) {
			...
			$status_filter_url   = remove_query_arg( [ 'paged', 's' ], $status_filter_url );
